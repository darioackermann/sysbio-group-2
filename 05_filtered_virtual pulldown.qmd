...
ggraph(filtered, layout = 'stress') + 
  geom_edge_link(aes(width = edge_between, alpha = edge_between, color = edge_between)) +
  geom_node_point(aes(shape = node_type, size = between, color = louvain)) +
  theme_void()
```
...
# Copy vertex attributes by name
V(subg)$between   <- V(filtered)$between[ match(V(subg)$name, V(filtered)$name) ]
V(subg)$node_type <- V(filtered)$node_type[ match(V(subg)$name, V(filtered)$name) ]

sub_edges <- ends(subg, E(subg), names = TRUE)
filt_edges <- ends(filtered, E(filtered), names = TRUE)

# Create an edge key like "A|B"
edge_key_sub  <- paste(sub_edges[,1],  sub_edges[,2],  sep="|")
edge_key_filt <- paste(filt_edges[,1], filt_edges[,2], sep="|")

# Match to filtered edges
idx <- match(edge_key_sub, edge_key_filt)

# Copy attribute
E(subg)$edge_between <- E(filtered)$edge_between[idx]

ggraph(subg, layout = 'stress') + 
  geom_edge_link(aes(width = edge_between, alpha = edge_between, color = edge_between)) +
  geom_node_point(aes(shape = node_type, size = between, color = node_type)) +
  theme_void()
```
...
V(subg)$between <- V(filtered)$between[  V(subg)]
V(subg)$node_type <- V(filtered)$node_type[  V(subg)]
E(subg)$edge_between <- E(filtered)$edge_between[  E(subg)]
